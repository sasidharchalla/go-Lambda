...
func listPurchaseOrders(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	var param string
	if input.Filter == "" {
		param = " order by poidsno desc"
	} else {
		param = "where " + input.Filter + " order by poidsno desc"
	}

	log.Println("filter Query :", param)
	var rows *sql.Rows
	var allPurchaseOrderDetails []PurchaseOrderDetails
	sqlStatement1 := `select poid,pono,podate,vendorname,status,pocat,vendortype,currencycode,povalue,total_quantity,gc_itemname from dbo.PurchaseOrdersGrid %s`
